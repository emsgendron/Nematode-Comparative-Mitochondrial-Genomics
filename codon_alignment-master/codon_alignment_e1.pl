...
`$Bin/muscle -align $ARGV[0].pep -output $ARGV[0].pep.aln`;							# Run MUSCLE alignment
print "Step 2 of 6: aa MUSCLE alignment finished\n";

system("perl $Bin/final_cleanup_e1.pl $ARGV[0].pep.aln $ARGV[2]");							# Calculate the percentage of gaps for each sequence
...
`$Bin/muscle -align $ARGV[0].pep.aln.cleanup -output $ARGV[0].pep.aln.cleanup.aln`;				# After removing sequences don't meet the criteria, sequences are aligned again
